...
from app.domain.services.dataset_builder import ML_FEATURE_COLUMNS, REGIME_FEATURE_COLUMNS, row_to_ml_features
...
async def _save_and_wrap(
    repository: AIPredictionRepository, model_name: str, symbol: str, prediction_type: str, output: dict
) -> ModelPrediction:
    ...
    return ModelPrediction(
        model_name=model_name, prediction_type=prediction_type, output=output, created_at=saved.created_at
    )
...
async def predict_market_with_features(
    ...
    candle_responses = await get_historical_candles(symbol, interval, 100, market_repository, client)
    candles = [
        Candle(
            symbol=symbol,
            interval=interval,
    ...
    if ml_features is None or any(value is None for value in ml_features.values()):
        raise ValueError("Insufficient indicator warm-up data to generate a prediction for the latest bar")

    feature_frame = pd.DataFrame([ml_features])
    key = symbol.replace("/", "_") + f"_{interval}"
    predictions: list[ModelPrediction] = []
    session = prediction_repository.session

    trend_model, _ = await load_latest_model(session, f"trend_classifier_{key}")
    ...
    if trend_model is not None:
        ...
        trend_output = {
            ...
            "probabilities": {TREND_LABEL_MAP[i]: float(probabilities[i]) for i in range(len(probabilities))},
        }
    predictions.append(await _save_and_wrap(prediction_repository, "trend_classifier", symbol, "trend", trend_output))

    opportunity_model, _ = await load_latest_model(session, f"entry_quality_{key}")
    opportunity_output = {"error": "model not trained yet"}
    if opportunity_model is not None:
        probability = float(opportunity_model.predict_proba(feature_frame[ML_FEATURE_COLUMNS])[0][1])
        opportunity_output = {"opportunity_probability": probability}
    predictions.append(
        await _save_and_wrap(prediction_repository, "entry_quality", symbol, "entry_quality", opportunity_output)
    )

    confidence_model, _ = await load_latest_model(session, f"confidence_scoring_{key}")
    ...
    predictions.append(
        await _save_and_wrap(prediction_repository, "confidence_scoring", symbol, "confidence", confidence_output)
    )

    risk_model, _ = await load_latest_model(session, f"risk_prediction_{key}")
    ...
    predictions.append(await _save_and_wrap(prediction_repository, "risk_prediction", symbol, "risk", risk_output))

    reward_model, _ = await load_latest_model(session, f"reward_prediction_{key}")
    ...
    predictions.append(
        await _save_and_wrap(prediction_repository, "reward_prediction", symbol, "reward", reward_output)
    )

    regime_model, _ = await load_latest_model(session, f"market_regime_{key}")
    ...
    predictions.append(await _save_and_wrap(prediction_repository, "market_regime", symbol, "regime", regime_output))
...
async def predict_market_use_case(
    ...
    response, _ = await predict_market_with_features(symbol, interval, market_repository, prediction_repository, client)
    return response